...
class DataWater
	...
	show: ()->
		@node = @createDom() if not Q('.data-waiter')
		@node.removeClass('hidden')
		self = @
		setTimeout( ->
			self.node.addClass('waiting')
		, 0)
		@
	...
	close: () ->
		h = (e) ->
			...
			target.off('transitionend',h,no)
			target.off('webkitTransitionEnd',h,no)
		if @node
			@node.removeClass('waiting')
			@node.on('transitionend',h,no)
			@node.on('webkitTransitionEnd',h,no)
		@

	remove: () ->
		self = @
		document.body.removeChild(self.node) if self.node
		self
