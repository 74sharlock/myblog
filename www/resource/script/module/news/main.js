...
//    Mobilebone.init();
    ...
    window.eventInit = function (a) {
        var $a = $(a),
            ...
            $categoryNav = $("#categoryNav a[class='active']", $a),
            $page = $('.page');
        $categoryNav.focus();
        if($mask.is(':visible')){
            $mask.hide();
        ...
        require('global/clickLoadMore')({
            callback:function(data){
                ...
                if(next[0]){
                    ...
                    var noDataItem = $('.no-data-2');
                    next.eq(0).remove();
                    if (noDataItem[0]) {
                        $('.load-more').remove();
                        noDataItem.removeClass('active');
                        setTimeout(function(){
                            noDataItem.addClass('active').on(event.transitionEnd,function(){
                                $(this).remove();
                            });
                        },5000);
                    }
